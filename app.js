...
const googleClientId = "522887260780-2rpev1qjgulbfr41ktcaea8d3ht8sj5m.apps.googleusercontent.com";
...
const selectors = {
  ...
  toggleGuestPassword: document.getElementById("toggleGuestPassword"),
  legacyNickPane: document.querySelector('[data-pane="nickname"]'),
...
const persistLockSetting = (value) => localStorage.setItem("slc2_lock_on_logout", value ? "1" : "0");
const loadLockSetting = () => localStorage.getItem("slc2_lock_on_logout") === "1";

const updateHeaderVisibility = (loggedIn) => {
  toggleHidden(selectors.logoutBtn, !loggedIn);
};
...
const isBlocked = (uid) => !!state.profile?.blocked?.[uid];
...
const nicknameLogin = async () => {
  ...
  saveSession({ uid, displayName: name, authType: "nickname", passwordHash: hashed });
  afterLogin();
};
...
const googleLogin = async () => {
  try {
    ...
    provider.setCustomParameters({ prompt: "select_account", client_id: googleClientId });
...
const generateGuestUid = (name, passHash) => {
  if (passHash) return `guest_${hashPassword(name + passHash).slice(0, 10)}`;
  return `guest_${Math.random().toString(36).slice(2, 8)}`;
};

const guestLogin = async () => {
  const name = selectors.guestNameInput.value.trim() || "Guest";
  const pass = selectors.guestPasswordInput.value.trim();
  const passHash = pass ? hashPassword(pass) : "";
  const uid = generateGuestUid(name || "Guest", passHash);
  await ensureUserProfile(uid, { displayName: name, passwordHash: passHash, google: false });
  state.user = { uid, displayName: name, authType: "guest" };
  saveSession({ uid, displayName: name, authType: "guest", passwordHash: passHash });
  afterLogin();
};
...
const restoreSession = async () => {
  ...
  if (!session) return;
  const profileSnap = await db.ref(`users/${session.uid}`).once("value");
  if (!profileSnap.exists()) {
    await ensureUserProfile(session.uid, { displayName: session.displayName || "Guest", passwordHash: session.passwordHash || "" });
  }
...
const bindUI = () => {
  ...
  selectors.toggleGuestPassword.onclick = () => {
    const type = selectors.guestPasswordInput.type === "password" ? "text" : "password";
    selectors.guestPasswordInput.type = type;
  };
  selectors.googleLoginBtn.onclick = googleLogin;
  selectors.guestLoginBtn.onclick = guestLogin;
  ...
  selectors.lockOnLogout.onchange = () => persistLockSetting(selectors.lockOnLogout.checked);
...
const afterLogin = () => {
  ...
  loadTheme();
  selectors.lockOnLogout.checked = loadLockSetting();
  attachChatStreams();
  loadRequests();
...
const renderMessage = (id, data) => {
  if (isBlocked(data.sender)) return;
  if (selectors.messages.querySelector(`[data-mid="${id}"]`)) return;
...
const findUserByNameOrId = async (input) => {
  const directSnap = await db.ref(`users/${input}`).once("value");
  if (directSnap.exists()) return { uid: input, profile: directSnap.val() };
  const nickId = input.startsWith("nick_") ? input : `nick_${input.toLowerCase()}`;
  const nickSnap = await db.ref(`users/${nickId}`).once("value");
  if (nickSnap.exists()) return { uid: nickId, profile: nickSnap.val() };
  const allSnap = await db.ref("users").once("value");
  const all = allSnap.val() || {};
  const match = Object.entries(all).find(([, val]) => (val.displayName || "").toLowerCase() === input.toLowerCase());
  if (match) return { uid: match[0], profile: match[1] };
  return null;
};
...
const openAddFriend = () => {
  openModal("Add Friend", (body) => {
    ...
    body.querySelector("#friendSendBtn").onclick = async () => {
      ...
      const target = await findUserByNameOrId(friend);
      ...
      const newRef = db.ref(`friendRequests/${target.uid}`).push(payload);
      db.ref(`friendRequestsSent/${state.user.uid}/${newRef.key}`).set({ to: target.uid, toName: target.profile.displayName || friend, createdAt: Date.now() });
      toast("Request sent");
      closeModal();
    };
  });
  ...
  openModal(type === "group" ? "Join / Create Group" : "Start Chat", (body) => {
    ...
    body.querySelector("#startChat").onclick = async () => {
      ...
      if (type === "group") {
        ...
        const target = await findUserByNameOrId(val);
        if (!target) return toast("User not found");
        const chatId = [state.user.uid, target.uid].sort().join("_");
        await db.ref(`privateChats/${chatId}`).update({
          participants: { [state.user.uid]: true, [target.uid]: true },
          name: target.profile?.displayName || val,
          lastMessage: Date.now()
        });
        toast("Chat ready");
      }
...
const init = () => {
  ...
  auth.onAuthStateChanged(async (user) => {
    if (user && !state.user) {
      await ensureUserProfile(user.uid, { displayName: user.displayName || user.email || "Google User", google: true });
      state.user = { uid: user.uid, displayName: user.displayName || user.email, authType: "google" };
